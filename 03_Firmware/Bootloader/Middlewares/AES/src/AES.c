...
const unsigned char aes_Rcon[14] = {0x01,
                                    0x02,
                                    0x04,
                                    0x08,
                                    0x10,
...
//Rows：      数组第几行  0~3
//Number:     左移字节数  1~3
void ShiftByteNumRows(unsigned char *SrcInOut,
                      unsigned char  Rows,
...
void Aes_Bytes_Xor(unsigned char *Byte_IO,
         ...
         Count++)
    {
        ...
        else
        {
            ...
            if (((Count % (KeySize / 4)) == 4) &&
                (KeySize ==
                 32)) //在32字节秘钥下  如果当前轮数为4的倍数  则要经过一次S盒替代
            {
