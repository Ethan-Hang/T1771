...
static uint8_t       s_u8_BufferIndex = BUFFER_A;  // 当前使用的缓冲区索引
...
static int32_t       Receive_Byte(uint8_t *c, uint16_t length, uint32_t timeout)
{
    ...
    if (pdTRUE ==
        xQueueReceive(Q_YmodemReclength, &s_u16_YmodRecLength, timeout))
    {
        return 0;
    }
    return -1;
...
int32_t Ymodem_Receive(uint8_t (*buf)[1030])
{
    ...
    // 初始化为 A 缓冲区
    s_u8_BufferIndex = BUFFER_A;
    packet_data = buf[s_u8_BufferIndex];
                            ...
                            else
                            {
                                if (packets_received == 0)
                                {
                                    ...
                                    if (packet_data[PACKET_HEADER] != 0)
                                    {
                                        ...
                                        int32_t * pu32_size = &size;
                                        // size = (uint32_t)atoi((char
                                        // *)file_size);
                                        ...
                                        // A/B 缓冲区切换
                                        // if (s_u8_BufferIndex == BUFFER_A)
                                        // {
                                        //     s_u8_BufferIndex = BUFFER_B;
                                        // }
                                        // else
                                        // {
                                        //     s_u8_BufferIndex = BUFFER_A;
                                ...
                                else
                                {
                                    ...
                                    xQueueSend(Queue_AppDataBuffer,
                                               &p_data,
                                               0);
                                    xSemaphoreTake(Semaphore_ExtFlashState,
                                                   portMAX_DELAY);
